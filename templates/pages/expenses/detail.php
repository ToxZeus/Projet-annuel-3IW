...
<section class="section">
    ...
    <article class="detail-card">
        ...
        <form method="post" action="/?page=expense&id=<?= $expense['id'] ?>" class="detail-form"><?= CsrfHelper::field() ?>
            ...
            <label<?= $frequency === 'ponctuel' ? ' class="hidden"' : '' ?> data-frequency-enddate>
                Date de fin
                <input type="date" name="end_date" value="<?= htmlspecialchars($old['endDate'] ?? ($expense['end_date'] ?? ''), ENT_QUOTES, 'UTF-8') ?>"<?= $frequency === 'ponctuel' ? ' disabled' : '' ?>>
            </label>

            <button class="button" type="submit">Mettre à jour</button>
        </form>

        <form method="post" action="/?page=expense&id=<?= $expense['id'] ?>" class="danger-form" style="margin-top:18px;"><?= CsrfHelper::field() ?>
