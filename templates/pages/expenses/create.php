...
<section class="section">
    <div class="section-header">
        <div>
            <p class="eyebrow">Nouvelle dépense</p>
            <h1>Créer une dépense pour <?= htmlspecialchars($account['short_name'] ?? '', ENT_QUOTES, 'UTF-8') ?></h1>
        </div>
        <a class="button button-secondary" href="/?page=account&id=<?= $account['id'] ?>">← Retour</a>
    </div>

    <article class="detail-card">
        <form method="post" action="/?page=expenses" class="detail-form"><?= CsrfHelper::field() ?>
            ...
            <button class="button" type="submit">Créer la dépense</button>
        </form>
    </article>
</section>
